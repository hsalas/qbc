...
if __name__ == '__main__':
	...
	#Run fo dn/(dz/dw) vs ew (In development)
	...
	elif m==6:	#load results from dn/dz vs ew in development
		dir_name = '../saved_files/dndz_v_ew/'+grid+'-'+limit_by+'-mass_10e'+str(np.log10(min_mass.value))+'_to_10e'+str(np.log10(max_mass.value))+'-rew_'+str(min_EW.value)+'_to_'+str(max_EW.value)+'-z_{:.2f}_to_{:.2f}'.format(min_z, max_z)+'-'+sn_tag
		create_dir(dir_name)	
		...
		if z_cl_type == 'spec' or z_cl_type == 'all':
			...
			create_dir(dir_name_outer)
			try:
				with open(dir_name_inner + '/results_'+str(dd)+'.pickle', 'rb') as f:
					results_spec_inner = pickle.load(f)
			except UnicodeDecodeError:
				with open(dir_name_outer + '/results_'+str(dd)+'.pickle', 'rb') as f:
					results_spec_outer = pickle.load(f, encoding='latin1')
			...
			print('\nResults outer:\n')
			print(results_spec_inner)
			plot_results_show(fig, 'ew', results_spec_inner, color='blue')
			legend_spec = 'mass {} to {}, ew {} to {}, z {} to {}, s {}, {}, {}'.format(np.log10(min_mass.value), np.log10(max_mass.value), min_EW, max_EW, min_z, max_z, s, 'phot', limit_by)
			legend.append(legend_spec)
			plt.legend(legend, loc='best', fontsize='medium' , numpoints=1)
			plt.show()
			fig.clf	
		...
		if z_cl_type == 'phot' or z_cl_type=='all':
			...
			try:
				with open(dir_name_inner + '/results_'+str(dd)+'.pickle', 'rb') as f:
					results_phot_inner = pickle.load(f)
			except UnicodeDecodeError:
				with open(dir_name_outer + '/results_'+str(dd)+'.pickle', 'rb') as f:
					results_phot_outer = pickle.load(f, encoding='latin1')
			fig = plt.figure()
			legend = []
		
			print('\nResults inner:\n')
			print(results_spec_inner)
			plot_results_show(fig, 'ew', results_phot_inner, color='red')
			legend_phot = 'mass {} to {}, ew {} to {}, z {} to {}, s {}, {}, {}'.format(np.log10(min_mass.value), np.log10(max_mass.value), min_EW, max_EW, min_z, max_z, s, 'spec', limit_by)
			legend.append(legend_phot)
			plt.legend(legend, loc='best', fontsize='medium' , numpoints=1)
			plt.show()
			fig.clf	
